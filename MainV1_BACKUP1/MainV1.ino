...
/*
  ...
  void fullExtension(){
    ...
    //% while at the same time adjust the servo motor angle to allow g-robot remain level

    
    //% when realsense find suitable location on platform
    
    
    //% lower the g-robot via P16 extension while ensuring the g-robot is parallel to ground
    do {                                               
      if (FEESER_pos < 85 || FEESER_pos >95){
        FEESER.write(90);       
    }
    while (){
      P16.write(0);
